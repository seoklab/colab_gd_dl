...
def create_gd_dl_in_file(program_dir: Path,
    ...
    with output_file_name.open('w') as f:
        f.write('%-21s %s\n'%('data_directory',str(program_dir/'data')))
        f.write('%-21s %s\n'%('top_type','polarh'))
        f.write('%-21s %s\n'%('print_level','30'))
        ...
        for line in f:
            if '@<TRIPOS>ATOM' in line:
                ...
                try:
                    ...
                    if tmp_sybyl == 'o':
                        group,period,sybyl_features = sybyl_to_lig['o.2']
                    else:
                        raise Exception(f'atom type error in {str(ligand_mol2)} as {tmp_sybyl}')
                tmp_node = torch.zeros(N_NODE)
                tmp_node[0] = 1.0
...
def preprocess_for_docking(args):
    protein_pdb, ligand_mol2, center_coord, out_dir, i, box_size, n_bank, max_opt_cycle, n_seed_cycle, is_print_bank = args
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', type=Path, required=True, help='protein.pdb location')
    parser.add_argument('-l', type=Path, required=True, help='ligand.mol2 location')
    parser.add_argument('-x', type=float, required=True, help='Center coordinate of x')
    parser.add_argument('-y', type=float, required=True, help='Center coordinate of y')
    parser.add_argument('-z', type=float, required=True, help='Center coordinate of z')
    ...
    parser.add_argument('--out_dir', type=Path, default=Path(os.getcwd()), help='location of output directory')
    
    parser.add_argument('--print_bank', type=str2bool, default=False, help='For development')

    parse_args = parser.parse_args()

    center_coord = (parse_args.x, parse_args.y, parse_args.z)

    out_dir = parse_args.out_dir

    args = (parse_args.p.resolve(),
            parse_args.l.resolve(),
            center_coord,
            out_dir.resolve(),
